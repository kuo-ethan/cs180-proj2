...
# Orapple
# l = np.zeros((300, 150, 3), dtype=np.uint8)
# r = np.ones((300, 150, 3), dtype=np.uint8) * 255.0
# right_mask = np.concatenate((l, r), axis=1).astype(np.uint8)
# left_mask = (255.0 - right_mask).astype(np.uint8)
# apple = skio.imread(f'../data/apple.jpeg')
# orange = skio.imread(f'../data/orange.jpeg')
# save_image(multi_resolution_blend(left_mask, right_mask, apple, orange), 'apple_orange_blend')
...
save_image(multi_resolution_blend(white_ball, black_ball, energy_ball, jumpshot), 'jumpshot_ball_blend')